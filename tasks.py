...
def docs(c):
    ...
    """
    c.run("rm -f docs/nobinobi-stats.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ nobinobi_stats")
...
def release(c, bumpsize=''):
    ...
    import nobinobi_stats
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=nobinobi_stats.__version__))
    c.run("git push --tags")
